...
class Coupled_Grid(wx.Frame):
    ...
    def RunCase(self, e):
        ...
        ip.replace_input("OVERLAP_EXTENTS", [1,nx, 1,3, 1,nz])
        ...
        for key in CFDDict:
            ip.replace_input(key, CFDDict[key])

        nproc = px*py*pz
        sp.Popen("mpiexec -n " + str(nproc) + " python ./CFD.py : -n 4 ./fortran/md", shell=True)
...
#        cfd.finalise()
        ...
        #Change MD input  
        #sp.Popen("mpiexec -n 4 ./fortran/md", shell=True)
