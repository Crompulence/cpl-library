...
// Constructor taking in ndArrayData vector
//template<class T> CPL::ndArray<T>::ndArray
//(
//    const int nd,
//    const int shape[],
//    const char ordering,//=defaultOrder
//    std::vector<T> ndArrayDatain
//)
//{
//    this->resize(nd, shape);
//    order = ordering;
...
// Sum all elements
template<class T> double CPL::ndArray<T>::sum()
{
    double sum = 0.0;
    int count = 0;
    for (int i=0; i<nElements; i++)
...
// Sum along axis
template<class T> std::vector<T> CPL::ndArray<T>::sum(const int dim)
{

    std::vector<int> indx = {0,1,2,3};
    indx.erase (indx.begin() + dim);

    double sum = 0.0;
    int count = 0;
    std::vector<T> newndArrayData;
    if (nDims < dim){
            throw shapeEx;
    }
    if (nDims == 4){
        int nElements = shapeVector[indx[0]]*shapeVector[indx[1]]*shapeVector[indx[2]];
        newndArrayData.resize(nElements);

        for (int i1=0; i1<shapeVector[indx[0]]; i1++) {
        for (int i2=0; i2<shapeVector[indx[1]]; i2++) {
        for (int i3=0; i3<shapeVector[indx[2]]; i3++) {
            sum = 0.0; count = 0;
            for (int i=0; i<shapeVector.at(dim); i++)
            {
                if (dim == 0) {
                    sum += ndArrayData[flatIndex(i, i1, i2, i3)];
                } else if (dim == 1) {
                    sum += ndArrayData[flatIndex(i1, i, i2, i3)];
                } else if (dim == 2) {
                    sum += ndArrayData[flatIndex(i1, i2, i, i3)];
                } else if (dim == 3) {
                    sum += ndArrayData[flatIndex(i1, i2, i3, i)];
                }
                count += 1;
            }
            newndArrayData[flatIndex(i1, i2, i3)] = sum/count;
        }}}
        int newshape[3] = {indx[0], indx[1], indx[2]};
        //CPL::ndArray<T> newndArray(newndArrayData, nDims-1, newshape, order);
        return newndArrayData;
    } else if (nDims == 3){
        int nElements = shapeVector[indx[0]]*shapeVector[indx[1]];
        newndArrayData.resize(nElements);

        for (int i1=0; i1<shapeVector[1]; i1++) {
        for (int i2=0; i2<shapeVector[2]; i2++) {
            sum = 0.0; count = 0;
            for (int i=0; i<shapeVector.at(dim); i++)
            {
                if (dim == 0) {
                    sum += ndArrayData[flatIndex(i, i1, i2)];
                } else if (dim == 1) {
                    sum += ndArrayData[flatIndex(i1, i, i2)];
                } else if (dim == 2) {
                    sum += ndArrayData[flatIndex(i1, i2, i)];
                }
                count += 1;
            }
            newndArrayData[flatIndex(i1, i2)] = sum/count;
        }}
        return newndArrayData;

    } else if (nDims == 2){
        int nElements = shapeVector[indx[0]];
        newndArrayData.resize(nElements);
        for (int i1=0; i1<shapeVector[1]; i1++) {
            sum = 0.0; count = 0;
            for (int i=0; i<shapeVector.at(dim); i++)
            {
                if (dim == 0) {
                    sum += ndArrayData[flatIndex(i, i1)];
                } else if (dim == 1) {
                    sum += ndArrayData[flatIndex(i1, i)];
                }
                count += 1;
            }
            newndArrayData[flatIndex(i1)] = sum/count;
        }
        return newndArrayData;
        ...
        for (int i=0; i<shapeVector.at(dim); i++)
        {
            sum += ndArrayData[flatIndex(i)];
            count += 1;
        }
        newndArrayData[0] = sum/count;
    }
    return newndArrayData;
}
